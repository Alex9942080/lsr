перевод 
Напишите оболочку в cи
 
Существуют программы, которые каждый использует каждый, и легко поставить своих разработчиков на пьедестал. 
Хотя разработка крупных программных проектов не взволнован, то базовая идея этого программного обеспечения довольно проста. 
Реализация самостоятельно - это интересный способ показать, что у вас есть то, что нужно, чтобы стать настоящим программистом. 
Итак, это прогулка о том, как я думаю, что моя упрощенная оболочка Unix, в надежде, что другие люди тоже так.

Остерегайтесь студентов университета! Во многих классах есть задания, которые просят вас написать оболочку, а некоторые преподаватели - 
это Awaare этого учебника и кода. Если вы студент в таком классе, вам следует скопировать (или скопировать, затем изменить) этот код без разрешения. 
И даже тогда я бы продвигался против того, чтобы полагаться на этот учебник.

Основная жизнь оболочки 

Давайте посмотрим на оболочку сверху вниз. Оболочка делает три основных вещах в своей жизни.

Инициализировать: на этом этапе типичная оболочка будет читать и выполнять свои файлы конфигурации. Это изменение аспектов поведения оболочки.
Интерпретация: Далее, оболочка считывает команды из stdin (что может быть интерактивным, или файл) и выполняет их.
Завершение: после выполнения его команд оболочка выполняет любые команды выключения, освобождает любую память и завершает.
Эти шаги настолько общие, что они могут применить ко многим программам, но мы будем использовать их для основы для нашей оболочки. 
Наша оболочка будет настолько проста, что нет файлов конфигурации, и нет команды выключения. Итак, мы просто вызовым функцию цикла, а затем прекратим. 
Но с точки зрения архитектуры, важно помнить, что время жизни программы больше, чем просто зацикливание.
...
Здесь вы можете видеть, что я только что придумал функцию, LSH_LOOP (), которая будет цикл, интерпретируя команды. Мы увидим реализацию этого следующего.

Основная петля оболочки

Поэтому мы позаботились о том, как программа должна начать. 
Теперь для основной логики программы: что делает оболочка во время своей петли? Ну, простой способ обработки команд - с тремя шагами:

Read: прочитайте команду с стандартного ввода.
Parse: разделить строку команд на программу и аргументы.
Execute: запустите аналитическую команду.
Здесь я переведу эти идеи в код для lsh_loop ():
...
Давайте пройдемся по коду. Первые несколько строк - просто объявления. Цикл Do-Wheele является более монастырем для проверки переменной статуса, 
поскольку он выполняется перед проверкой своего значения. В цикле мы распечатываем подсказку, вызовываем функцию, чтобы прочитать строку, 
вызовать функцию, чтобы разделить строку на ARG, и выполнить ARG. Наконец, мы освобождаем линию и аргументы, которые мы создаем ранее. 
Обратите внимание, что мы используем переменную состояния, возвращаемую LSH_EXECUTE (), чтобы определить, когда выйти.

Reading a line

Reading a line from stdin sounds so simple, but in C it can be a hassle. 
The sad thing is that you don’t know ahead of time how much text a user will enter into their shell. 
You can’t simply allocate a block and hope they don’t exceed it. Instead, you need to start with a block, 
and if they do exceed it, reallocate with more space. This is a common strategy in C, and we’ll use it to implement lsh_read_line().
...
Footnotes
Edit 4/Footnote: It’s 2021, over 6.5 years since writing this tutorial. I now work on operating systems in C for a living. I just wanted to say that I still do not remember how to declare a function pointer. I still need to Google it every time. ↩
